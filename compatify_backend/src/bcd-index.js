...
function safeLookupByToken(bcdIndex, token, prefix) {
  ...
  // - For JS/API lookups don't allow ultra-short tokens
  ...
  const filterByPrefix = (keys) => {
    if (!prefixes || !prefixes.length) return keys;
    return keys.filter(k => prefixes.some(p => k.startsWith(p)));
  };

  // 1) Prefer exact last-segment matches (most precise). If any exist -> return them.
  const exactLast = filterByPrefix(bcdIndex.lastSegmentMap.get(lower) || []);
  if (exactLast && exactLast.length) return Array.from(new Set(exactLast));

  // 2) Otherwise gather minimal candidates from tokenMap & segmentMap
  let candidates = Array.from(new Set([
    ...(bcdIndex.tokenMap.get(lower) || []),
    ...(bcdIndex.segmentMap.get(lower) || [])
  ]));
  candidates = filterByPrefix(candidates);

  // 3) Prefer candidates where token appears near the tail (last 2 segments)
  const tailPreferred = [], tailAllowed = [];
  for (const k of candidates) {
    const segs = k.split('.').map(s => s.toLowerCase());
    (segs.slice(-2).includes(lower) ? tailPreferred : tailAllowed).push(k);
  }
  let result = tailPreferred.length ? tailPreferred : tailAllowed;

  // 4) HTML elements: prefer direct html.elements.NAME
  if (prefixes?.includes('html.elements')) {
    const direct = result.filter(k => {
      const segs = k.split('.');
  ...
  // 5) CSS properties: require last segment to include all parts of token
  if (prefixes && prefixes.some(p => p === 'css.properties')) {
    const tokenParts = lower.split(/[-_]/).filter(Boolean);
    result = result.filter(k => {
      const lastSeg = k.split('.').pop().toLowerCase();
      const lastParts = lastSeg.split(/[-_]/).filter(Boolean);
      // require that every token part is present in last segment tokens
      return tokenParts.every(tp => lastParts.includes(tp));
    });
  }
...
function parseJSWithAcorn(code, bcdIndex, addMatch) {
  ...
  const ambiguousPrototypeMethods = new Set([
    'includes', 'matchall', 'match', 'repeat', 'indexof', 'find', 'filter', 'map',
    'reduce', 'keys', 'values', 'normalize' // added normalize (multi-host)
  ]);
...
function lookupAndAdd(parts, opts = {}) {
  ...
  let ownerHint = opts.ownerHint ? String(opts.ownerHint) : null;
  ...
  const last = partsArr[partsArr.length - 1].toLowerCase();

  // If ownerHint looks like a constructor name (e.g. 'Array' or 'Element:div'), normalize it for checks
  const ownerLower = ownerHint ? ownerHint.toLowerCase() : null;

  // 0) If token is obviously ambiguous and we have no owner, skip aggressively
  if (!ownerLower && ambiguousPrototypeMethods.has(last) && partsArr.length === 1) return;

  // 1) Owner-aware last-segment match (handles cases like String.prototype.normalize)
  if (ownerLower) {
    // look for keys whose last segment === last AND owner is near tail
    const lastSegmentCandidates = (bcdIndex.lastSegmentMap.get(last) || []).filter(k => {
      // apply prefixes filter: we only want JS/API candidates here
      return (k.startsWith('api') || k.startsWith('javascript.builtins')) && containsOwnerNearTail(k, ownerLower);
    });

    if (lastSegmentCandidates.length) {
      lastSegmentCandidates.forEach(k => addMatch(k, partsArr.join('.'), Math.max(confidence, 0.98), `${reason}-owner-lastseg`));
      return;
    }

    // If we have a chain (e.g. String.normalize), try a relaxed owner-aware chain match:
    const relaxed = bcdIndex.allKeys.filter(k => {
      if (!(k.startsWith('api') || k.startsWith('javascript.builtins'))) return false;
      const segs = k.split('.').map(s => s.toLowerCase());
      // last must match the member
      if (segs[segs.length - 1] !== last) return false;
      // require owner token somewhere near the tail (allows 'prototype' in-between)
      return containsOwnerNearTail(k, ownerLower) || segs.includes(ownerLower);
    });

    if (relaxed.length) {
      relaxed.forEach(k => addMatch(k, partsArr.join('.'), Math.max(confidence, 0.975), `${reason}-owner-relaxed`));
      return;
    }

    // If ownerHint exists but we found nothing, avoid broad exact fallbacks for ambiguous methods
    if (ambiguousPrototypeMethods.has(last)) return;
  }

  // 2) Exact chain matches (non-owner-aware)
  ...
  // 3) Single-part tokens: allow only if allowed and only exact last-segment matches (strict)
  if (partsArr.length === 1 && allowSingle) {
    const exactLast = (bcdIndex.lastSegmentMap.get(last) || []).filter(k => (k.startsWith('api') || k.startsWith('javascript.builtins')));
    if (exactLast.length) {
      // If ownerHint exists, further filter for owner proximity
      const use = ownerLower ? exactLast.filter(k => containsOwnerNearTail(k, ownerLower) || k.split('.').map(s => s.toLowerCase()).includes(ownerLower)) : exactLast;
      use.forEach(k => addMatch(k, last, Math.max(confidence, 0.92), `${reason}-single-exact`));
    }
    return;
  }

  // 4) Final -- very conservative fallback: only long tokens and owner-proxied keys
  if (String(last).length >= 4 && !ambiguousPrototypeMethods.has(last)) {
    let keys = safeLookupByToken(bcdIndex, last, ['api','javascript.builtins']);
    if (ownerLower) keys = keys.filter(k => containsOwnerNearTail(k, ownerLower) || k.split('.').map(s => s.toLowerCase()).includes(ownerLower));
...
function parseCSSWithPostCSS(cssText, bcdIndex, addMatch) {
  ...
  const ignoredValueTokens = new Set(['top','left','right','center','bottom']);

  root.walk(node => {
    if (node.type === 'decl') {
      const prop = normalizeIdentifier(node.prop || '');
      // strict property matches (last segment must match)
      ...
      // property-specific values (avoid broad token matches)
      if (node.value && valueParser) {
        const parsed = valueParser(node.value);
        parsed.walk(n => {
          if (n.type === 'word') {
            const tok = normalizeIdentifier(n.value);
            if (tok.length >= 3 && !ignoredValueTokens.has(tok)) {
              const candidates = [
    ...
    if (effective === 'css') {
      ...
      if (propMatch) {
        ...
        if (value && valueParser) {
          const valTokens = splitTokens(node.value)
            .filter(t => t.length >= 4 && !['top','left','right','center'].includes(t.toLowerCase()));
